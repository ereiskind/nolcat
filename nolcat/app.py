...
def create_app():
    ...
    app.config['UPLOAD_FOLDER'] = './relation_initialization_template'  # This config sets the file that handles both Flask file downloads and uploads, but since all input, including file uploads, is handled with WTForms, this folder is only used for storing static content the user will need to download.
    csrf.init_app(app)
    db.init_app(app)
