...
class FiscalYears(db.Model):
    ...
    def calculate_depreciated_ACRL_63(self):
        ...
        self._log.debug(return_value_from_query_statement(ACRL_63))
        return ACRL_63
    ...
    def calculate_ACRL_61b(self):
        ...
        self._log.debug(return_value_from_query_statement(ACRL_61b))
        return ACRL_61b
    ...
    def calculate_ARL_18(self):
        ...
        self._log.debug(return_value_from_query_statement(ARL_18))
        return ARL_18
    ...
    def calculate_ARL_19(self):
        ...
        self._log.debug(return_value_from_query_statement(ARL_19))
        return ARL_19
    ...
    def calculate_ARL_20(self):
        ...
        self._log.debug(return_value_from_query_statement(ARL_20))
        return ARL_20
    ...
    def create_usage_tracking_records_for_fiscal_year(self):
        ...
        self._log.info(f"Records being loaded into `annualUsageCollectionTracking`:\n{df.index}")
...
class StatisticsSources(db.Model):
        ...
        if isinstance(subset_of_months_to_harvest, str):
            ...
            if len(subset_of_months_to_harvest) == 0:  #ToDo: This is being left as not raising an error, returning null to represent no parquet files, since the `_check_if_data_in_database()` function will need to change to look in parquet files instead of querying MySQL
                ...
                for month_to_harvest in subset_of_months_to_harvest:
                    ...
                    if S3_file_name.suffix == ".parquet":
    ...
    def _check_if_data_in_database(self, report, start_date, end_date):
        ...
        self._log.debug(f"The months in the date range are {months_in_date_range}.")
        months_to_harvest = []
        ...
        for month_being_checked in months_in_date_range:
            ...
            if number_of_records == 0:
                ...
                self._log.warning(f"There were records for {self.statistics_source_name} in {month_being_checked.strftime('%Y-%m')} already loaded in the database; {month_being_checked.strftime('%Y-%m')} won't be included in the harvested date range.")
        
        self._log.info(f"The months to harvest are {months_to_harvest}.")
...
class ResourceSources(db.Model):
    ...
    def change_StatisticsSource(self, statistics_source_PK):
        ...
        if check_for_existing_record.empty:
            ...
            self._log.debug("Updating an existing record in the `statisticsResourceSources` relation.")
            update_statement=f"""
                UPDATE statisticsResourceSources
...
class COUNTERData(db.Model):
    ...
    resource_name = db.Column(db.String(3600))
    publisher = db.Column(db.String(425))
    publisher_ID = db.Column(db.String(150))
    platform = db.Column(db.String(135))
    authors = db.Column(db.String(4400))
    publication_date = db.Column(db.DateTime)
    article_version = db.Column(db.String(50))
    DOI = db.Column(db.String(110))
    ...
    parent_title = db.Column(db.String(3600))
    parent_authors = db.Column(db.String(4400))
    ...
    parent_DOI = db.Column(db.String(110))
    ...
    parent_URI = db.Column(db.String(450))
