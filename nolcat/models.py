...
class FiscalYears(db.Model):
    ...
    def create_usage_tracking_records_for_fiscal_year(self):
        ...
        current_statistics_sources_PKs = [(PK, self.fiscal_year_ID) for PK in current_statistics_sources.uniques().tolist()]  # `uniques()` method returns a numpy array, so numpy's `tolist()` method is used
