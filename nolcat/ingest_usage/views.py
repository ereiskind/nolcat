...
def harvest_SUSHI_statistics():
    ...
    if request.method == 'GET':
        ...
        logging.warning(f"`form.is_submitted()`: {form.is_submitted()}")
        logging.warning(f"`form.validate()`: {form.validate()}")
        logging.warning(f"`form.statistics_source.validate()`: {form.statistics_source.validate(form)}")
        logging.warning(f"`form.begin_date.validate()`: {form.begin_date.validate(form)}")
        logging.warning(f"`form.end_date.validate()`: {form.end_date.validate(form)}")
        return abort(404)
