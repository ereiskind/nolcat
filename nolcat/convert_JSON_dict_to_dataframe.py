...
class ConvertJSONDictToDataframe:
    ...
    def _transform_R5_JSON(self, report_creation_date):
        ...
        for record in self.SUSHI_JSON_dictionary['Report_Items']:
            log.debug(f"Starting iteration for new JSON record {record}.")  ##about_to_statement()
            ...
            for key, value in record.items():
                ...
                if key == "Database" or key == "Title" or key == "Item":
                    log.debug(f"Preparing to add {key} value `{value}` to the record.")  ##about_to_statement()
                ...
                # Null value handling isn't needed because all null values are removed
                    ...
                    for key_for_parent, value_for_parent in value.items():
                        ...
                        if key_for_parent == "Item_Name":
                            log.debug(f"Preparing to add {key_for_parent} value `{value_for_parent}` to the record.")  ##about_to_statement()
            ...
            log.debug(f"Preparing to add date, metric type, and usage count values `{performance}` to the record.")  ##about_to_statement()
