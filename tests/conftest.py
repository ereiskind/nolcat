...
def app():
    ...
    app = create_app()
    log.info(f"At start of `conftest.app()`, `app` is {app} (type {type(app)})")
    app.debug = True
    ...
    log.debug(f"After configs in `conftest.app()`, `app` is {app} (type {type(app)})")
    context = app.app_context()  # Creates an application context
    log.debug(f"At creation in `conftest.app()`, `context` is {context} (type {type(context)})")
    context.push()  # Binds the application context to the current context/Flask application
    log.info(f"Just before `yield` in `conftest.app()`, `app` is {app} (type {type(app)}) and `context` is {context} (type {type(context)})")
    yield app
    log.info("After `yield` in `conftest.app()`")
...
def db(app):
    ...
    _db.app = app
    log.info(f"At start of `conftest.db()`, `_db` is {_db} (type {type(_db)}) and `_db.app` is {_db.app} (type {type(_db.app)})")
    _db.create_all()
    log.info(f"Just before `yield` in `conftest.db()`, `_db` is {_db} (type {type(_db)})")
    yield _db
    log.info("After `yield` in `conftest.db()`")  # The statement appears in stdout after all the tests in the module, but the database isn't being rolled back
...
def session(engine, db):
    ...
    log.info(f"At start of `conftest.session()`, `connection` is {connection} (type {type(connection)})")
    ...
    log.info(f"At start of `conftest.session()`, `session` is {session} (type {type(session)})")

    #Subsection: Yield Fixture
    transaction = connection.begin()
    log.info(f"At start of `conftest.session()`, `transaction` is {transaction} (type {type(transaction)})")
    ...
    log.info(f"Just before `yield` in `conftest.session()`, `session` is {session} (type {type(session)}) and `db.session` is {db.session} (type {type(db.session)})")
    yield session

    #Section: Close and Remove Connections
    log.info('After `yield` in `conftest.session()`')  # The statement appears in stdout after all the tests in the module, but the database isn't being rolled back
    session.close()
