...
def test_create_dataframe(sample_COUNTER_report_workbooks, COUNTERData_relation):
    ...
    try:
        ...
        log.warning(f"Fields are equal: {df.columns == COUNTERData_relation.columns}")
        log.warning(f"Sorted fields are equal: {df.columns.sort() == COUNTERData_relation.columns.sort()}")
