...
def test_upload_COUNTER_reports(engine, client, header_value, COUNTERData_relation, caplog):
    ...
    assert re.search(r'Successfully loaded \d* records into the .* relation\.', prepare_HTML_page_for_comparison(POST_response.data))  # This confirms the flash message indicating success appears; if there's an error, the error message appears instead, meaning this statement will fail  ##Check-load_data_into_database
