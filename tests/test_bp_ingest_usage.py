...
def test_upload_COUNTER_data_via_Excel(engine, client, header_value, COUNTERData_relation, caplog):
    ...
    try:
        ...
        log.info(f"Fields are the same: {COUNTERData_relation.columns == COUNTERData_relation_data.columns}")
        try:
            COUNTERData_relation.index = COUNTERData_relation.index.astype('Int64Index')
            log.info(f"Final dataframe comparison after `COUNTERData_relation.index.astype('Int64Index')`:\n{COUNTERData_relation.compare(COUNTERData_relation_data)}")
        except:
            try:
                COUNTERData_relation.index = COUNTERData_relation.index.astype('Int64')
                log.info(f"Final dataframe comparison after `COUNTERData_relation.index.astype('Int64')`:\n{COUNTERData_relation.compare(COUNTERData_relation_data)}")
            except:
                a = COUNTERData_relation.copy()
                b = COUNTERData_relation.copy()
                a.index = a.index.astype('Int64Index')
                log.info(f"`COUNTERData_relation.index.astype('Int64Index')`:\n{a.index}")
