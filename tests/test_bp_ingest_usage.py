...
def test_harvest_SUSHI_statistics(engine, most_recent_month_with_usage, client, header_value):
    ...
    primary_key_list = change_single_field_dataframe_into_series(primary_key_list).astype('string')
    print(f"Before `apply`, `primary_key_list` is {primary_key_list}")
    primary_key_list = primary_key_list.apply(lambda string_of_float: string_of_float.split(".")[0] if not pd.isnull(string_of_float) else string_of_float).astype('string').to_list()  # String created is of a float (aka `n.0`), so the decimal and everything after it need to be removed; the transformation changes the series dtype back to object, so it needs to be set to string again
    print(f"After `apply`, `primary_key_list` is {primary_key_list}")
    form_input = {
