...
def test_upload_historical_non_COUNTER_usage(engine, client, header_value, files_for_test_upload_historical_non_COUNTER_usage, caplog):
    ...
    try:
        log.warning(f"`random.sample([dict.keys()], k=k)`: {format_list_for_stdout(random.sample([list_of_AUCT_submission_fields.keys()], k=k))}")
    except:
        pass
    try:
        log.warning(f"`random.sample([dict.values()], k=k)`: {format_list_for_stdout(random.sample([list_of_AUCT_submission_fields.values()], k=k))}")
    except:
        pass
    try:
        log.warning(f"`random.sample([dict.items()], k=k)`: {format_list_for_stdout(random.sample([list_of_AUCT_submission_fields.items()], k=k))}")
    except:
        pass
    try:
        log.warning(f"`random.choices([dict.keys()], k=k)`: {format_list_for_stdout(random.choices([list_of_AUCT_submission_fields.keys()], k=k))}")
    except:
        pass
    try:
        log.warning(f"`random.choices([dict.values()], k=k)`: {format_list_for_stdout(random.choices([list_of_AUCT_submission_fields.values()], k=k))}")
    except:
        pass
    try:
        log.warning(f"`random.choices([dict.items()], k=k)`: {format_list_for_stdout(random.choices([list_of_AUCT_submission_fields.items()], k=k))}")
    except:
        pass
    #TEST: end temp
