...
def test_GET_request_for_upload_historical_non_COUNTER_usage(client, caplog):
    ...
    GET_response_page_title = GET_soup.body.h1
    file_inputs = 0
    log.warning([x.repr() for x in GET_soup.find(name='input')])  #TEST: temp
...
def files_for_test_upload_historical_non_COUNTER_usage(tmp_path, caplog):
    ...
    if number_of_uploads % 2 == 1:
        Excel_files = random.choices([file for file in Path(TOP_NOLCAT_DIRECTORY, 'tests', 'bin', 'sample_COUNTER_R4_reports').iterdir()], k=(number_of_uploads//2)+1)
    else:
...
def test_upload_historical_non_COUNTER_usage(client, header_value, files_for_test_upload_historical_non_COUNTER_usage, caplog):
    ...
    form_submissions_fields = {k: ((v.name, open(v, 'rb')) if v.suffix == ".xlsx" else v) for k, v in used_submission_fields_and_file_paths.items()}
