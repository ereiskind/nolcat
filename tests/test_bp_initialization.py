...
def test_collect_initial_relation_data(tmp_path, header_value, client):
    ...
    print(f"`POST_request.charset`: {POST_request.charset}")  # `utf-8`
    print(f"`POST_request.content_type` (type {type(POST_request.content_type)}): {POST_request.content_type}")  # `text/html; charset=utf-8`
    print(f"`POST_request.data`: {POST_request.data}")  # `b'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">\n<title>400 Bad Request</title>\n<h1>Bad Request</h1>\n<p>The CSRF token is missing.</p>\n'`
    print(f"`POST_request.mimetype`: {POST_request.mimetype}")  # `text/html`
    print(f"`POST_request.mimetype_params` (type {type(POST_request.mimetype_params)}): {POST_request.mimetype_params}")
    print(f"`POST_request.response`: {POST_request.response}")  # `[b'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">\n<title>400 Bad Request</title>\n<h1>Bad Request</h1>\n<p>The CSRF token is missing.</p>\n']`
    print(f"`POST_request.status_code`: {POST_request.status_code}")  # `400`
